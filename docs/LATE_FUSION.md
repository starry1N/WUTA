...
通用融合入口的相机最近帧容差由其配置决定；当前实机入口为 30 ms，
LiDAR 等待相机/TF 最多 100 ms，队列 20 帧。橙锥调试旁路单独使用 60 ms
相机窗口，不进入地图。等待上限和采样时间差容差是两个独立条件；
实机参数见 [统一调参文件](../WUTA-FSD/ros2_ws/src/perception/config/field_tuning.yaml)。
