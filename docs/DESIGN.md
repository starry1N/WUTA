...
完整 ConeMap 快捷输入。真实系统仍应由相机检测与融合节点提供颜色。该节点当前为待实现项：
它应将稳定的蓝/黄分类融合进现有 `ConeArray`/`ConeMap` 链路，而不绕过在线建图。
...
| 紧凑赛道跨段歧义 | 待实现相机锥桶颜色融合，优先蓝/黄边界配对 | 相邻赛段仅以几何距离/航向在 LiDAR-only Delaunay 中不能始终区分；相机提供的语义颜色保留在现有 ConeArray/ConeMap 链路内，不引入 YAML 真值中心线 |
