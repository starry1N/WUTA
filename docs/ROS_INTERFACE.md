...
| `/kiss/odometry` | `nav_msgs/msg/Odometry` | `kiss_icp_node` | `ekf_node`、map_saver | 默认约 10 Hz；`odom` frame；KISS 不发布 TF；EKF 对 pose 使用 3σ 创新拒绝门限 |
...
| ekf_node | `odom0/odom1` 配置、KISS 3σ pose 门限、INS 5σ pose 门限、过程噪声 | `localization_manager/config/ekf.yaml`；直接融合连续 `/kiss/odometry` 与 INS 绝对锚点；创新门限拒绝重复赛段错误重定位。禁用实验性的间歇 KISS 差分门控，避免重接入时向 EKF 注入位姿跳变 |
